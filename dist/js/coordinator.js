...
import { renderWeekGrid, toast } from './ui.js';
...
function renderCoordinatorGrid() {
    renderWeekGrid('coord-week-grid', blockedSlots, bookingsList, 'coordinator', {
...
function renderCoordinatorStats() {
    const totalFree = Math.max(0, TOTAL_SLOTS * (currentTeacherId ? 1 : teachers.length) - blockedSlots.length - bookingsList.length);
...
function openEditModal(data) {
    ...
    if (info)
        info.textContent = `${tname} — ${dayLabel} às ${data.hour}`;
    // Campos
    const teacherInput = document.getElementById('coord-edit-teacher');
    const dayInput = document.getElementById('coord-edit-day');
    if (teacherInput)
        teacherInput.value = data.teacherId || '';
    if (dayInput)
        dayInput.value = data.day || '';
    const hourInput = document.getElementById('coord-edit-hour');
    if (hourInput)
        hourInput.value = data.hour || '';
    const studentNameInput = document.getElementById('coord-edit-student-name');
    const studentEmailInput = document.getElementById('coord-edit-student-email');
    if (studentNameInput)
    ...
    if (data.type === 'booked') {
        const booking = bookingsList.find(b => b.id === data.id);
        if (booking && studentNameInput)
            studentNameInput.value = booking.student_name || '';
        if (booking && studentEmailInput)
            studentEmailInput.value = booking.student_email || '';
        if (studentFields)
...
// ================================================================
async function htmlToCanvas(_gridEl) {

...
async function loadTeacherData(teacherId) {
    ...
    if (unsubscribeBooked)
        unsubscribeBooked();
    // Mostra loading
    const gridEl = document.getElementById('week-grid-teacher');
    if (gridEl) {
        gridEl.innerHTML = `
...
function renderTeacherGrid() {
    renderWeekGrid('week-grid-teacher', blockedSlots, bookings, 'teacher', {
...
async function handleSlotClick({ day, hour, status, id }) {
    openTeacherSlotModal({ day, hour, status, id }, async ({ action, hour: customHour, id: slotId }) => {
        try {
            if (action === 'block') {
                const result = await blockSlot(currentTeacherId, day, customHour);
                const newSlot = Array.isArray(result) ? result[0] : result;
...
function renderStats() {
    ...
    if (freeEl)
        freeEl.textContent = totalFree.toString();
    if (blockedEl)
        blockedEl.textContent = totalBlocked.toString();
    if (bookedEl)
        bookedEl.textContent = totalBooked.toString();
}
// ================================================================

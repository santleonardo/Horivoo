...
import { registerSW, setupInstallPrompt, setupNetworkStatus, isStandalone } from './pwa.js';
import { signIn, signUp, signOut, getSession, getUser, forgotPassword } from './auth.js';
...
const initialized = { teacher: false, student: false, coordinator: false };
function switchTab(tab) {
    document.querySelectorAll('.header-nav button').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.tab === tab);
    });
    const tabs = ['teacher', 'student', 'coordinator'];
    tabs.forEach(t => {
...
function showScreen(screenId) {
    const screens = ['config-screen', 'login-screen', 'app-main'];
    screens.forEach(id => {
...
function setupAuthUI() {
    ...
    document.getElementById('login-btn')?.addEventListener('click', async () => {
        ...
        try {
            ...
            const msg = Object.entries(map).find(([k]) => errMsg.includes(k))?.[1] || errMsg;
            toast(msg, 'error');
        }
        finally {
    ...
    document.getElementById('signup-btn')?.addEventListener('click', async () => {
        ...
        if (password.length < 6) {
            toast('Senha deve ter ao menos 6 caracteres.', 'error');
            return;
        }
        if (password !== confirm) {
        ...
        try {
            ...
            const errMsg = err.message || '';
            const msg = errMsg.includes('already registered')
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (wasCallback)
        toast('E-mail confirmado com sucesso!', 'success', 4000);
    // PWA
    await registerSW();
    setupNetworkStatus();
    setupInstallPrompt();
    ...
    document.querySelectorAll('.header-nav button[data-tab]').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (appMain && appMain.style.display !== 'none') {
                switchTab(btn.dataset.tab || 'teacher');
            }
        });
    });

...
export function AuthForm({ onAuthSuccess, onGuestMode }: AuthFormProps) {
  ...
  // Memoize Supabase client — safe even if env vars are missing
  const supabase = useRef<ReturnType<typeof createClient> | null>(null);
  if (supabase.current === null && isSupabaseConfigured()) {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      if (!client) { toast.error("Supabase não configurado."); return; }
